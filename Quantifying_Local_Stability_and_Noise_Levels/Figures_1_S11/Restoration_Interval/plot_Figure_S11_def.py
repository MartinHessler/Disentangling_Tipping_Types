...
markov_slope_uncorrected_data = np.load('BLE/slopes_without_outlier_correction.npy')
markov_noise_uncorrected_data = np.load('BLE/noise_without_outlier_correction.npy')
...
for i in range(3):
	if i == 0:
		...
		ax.text(x_fig_label, y_fig_label, 'd', transform=ax.transAxes, fontsize = text_size+fig_label_add, weight = 'bold')
		...
		ax.plot(time.iloc[window_size - 1 + loop_range], markov_slope_uncorrected_data[0,:], color = 'grey', label = r'$\hat{\zeta}$ with outliers', lw = line_width)
		
		ax.plot(time.iloc[window_size - 1 + loop_range], markov_slope[0,:], lw = line_width, color = 'b', label=r'$\hat{\zeta}_{\rm BLE}$',zorder = 5)
		ax.fill_between(time.iloc[window_size - 1 + loop_range], markov_slope[1,:], markov_slope[2,:], color = 'orange', alpha = 0.6,zorder = 4)
		ax.fill_between(time.iloc[window_size - 1 + loop_range], markov_slope[3,:], markov_slope[4,:], color = 'orange', alpha = 0.4,zorder = 4)
		...
		ax.ticklabel_format(axis="y", style="sci", scilimits=(0,0), useMathText=True)
		ax.get_yaxis().get_offset_text().set_visible(False)
		ax.text(x_pos_powertext,y_pos_powertext, r'$\times 10^{-3}$', transform=ax.transAxes, fontsize = power_text_factor*text_size, zorder = 100)
		...
		ax.plot(time.iloc[window_size - 1 + loop_range], markov_noise_uncorrected_data[0,:], color = 'grey', label = r'$\hat{\sigma}$ with outliers', lw = line_width, zorder =1)
		

		ax.plot(time.iloc[window_size - 1 + loop_range], markov_noise[0,:], color =  'b', lw  = line_width, label = r'$\hat{\sigma}$',zorder = 5)
		ax.fill_between(time.iloc[window_size - 1 + loop_range], markov_noise[1,:], markov_noise[2,:], color = 'orange', alpha = 0.6,zorder = 4)
		ax.fill_between(time.iloc[window_size - 1 + loop_range], markov_noise[3,:], markov_noise[4,:], color = 'orange',zorder = 4)
...
sublabelsize = 40
subpowersize = 30
x_pos_subaxispowertext,y_pos_subaxispowertext = 0.24, 2.83
x_pos_subaxispowertext2,y_pos_subaxispowertext2 = 0.7015, 2.83
...
sub2time = time_reconstructed.iloc[np.arange(condition_indices.size)[condition_indices]]
...
subaxis2.text(x_pos_subaxispowertext2,y_pos_subaxispowertext2, r'$\times 10^{-2}$', transform=ax.transAxes, fontsize = subpowersize)
...
plt.savefig('Figure_S11_def.pdf')
plt.show()
