...
for i in range(3):
	if i == 0:
		...
		ax.plot(time[int(window_size/2.) + loop_range], markov_slope[0,:], lw = 3, color = 'b')
		ax.plot(time, analytical_drift_slopes_to_plot, 'g', lw = 3)
		ax.fill_between(time[int(window_size/2.) + loop_range], markov_slope[1,:], markov_slope[2,:], color = 'orange', alpha = 0.6, label = r'CI $\epsilon$')
		ax.fill_between(time[int(window_size/2.) + loop_range],markov_slope[3,:], markov_slope[4,:], color = 'orange', alpha = 0.4)
		ax.plot(time[int(window_size/2.) + loop_range], nonmarkov_slope[0,:], lw = 3, color = 'r')
		ax.fill_between(time[int(window_size/2.) + loop_range], nonmarkov_slope[1,:],nonmarkov_slope[2,:], color = 'green', alpha = 0.6, label = r'CI $\epsilon$')
		ax.fill_between(time[int(window_size/2.) + loop_range],nonmarkov_slope[3,:],nonmarkov_slope[4,:], color = 'green', alpha = 0.4)
		ax.set_xticks([])
		ax.set_yticks([-20,0])
		...
		ax.plot(time[int(window_size/2.) + loop_range], markov_noise[1,:], color =  'b', lw = 3,)
		ax.fill_between(time[int(window_size/2.) + loop_range],markov_noise[1,:], markov_noise[1,:], color = 'orange', alpha = 0.6,)
		ax.fill_between(time[int(window_size/2.) + loop_range],markov_noise[1,:], markov_noise[1,:], color = 'orange', alpha = 0.4)
		ax.plot(time[int(window_size/2.) + loop_range], nonmarkov_noise[0,:], lw = 3, color = 'r')
		ax.fill_between(time[int(window_size/2.) + loop_range], nonmarkov_noise[1,:],nonmarkov_noise[2,:], color = 'green', alpha = 0.6)#, label = r'CI $\epsilon$')
