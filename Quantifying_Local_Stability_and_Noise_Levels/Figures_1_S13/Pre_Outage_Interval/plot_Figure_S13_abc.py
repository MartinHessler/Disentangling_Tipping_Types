...
for i in range(3):
	if i == 0:
		...
		ax.plot(time.iloc[window_size - 1 + loop_range], markov_slope[0,:], lw = line_width, color = 'b',label = r'$\hat{\zeta}$',zorder=5)
		...
		ax.set_ylim([-7.4,0.75])
		ax.set_yticks([-6,-4,-2,0])

		ax.axvspan(datetime.strptime(method_fingerprint_start, "%Y-%m-%d %H:%M:%S.%f"), datetime.strptime('1996-08-10 15:42:03.139', "%Y-%m-%d %H:%M:%S.%f"), alpha=0.3, color=interval_color, zorder = zorder_interval_markers)
		ax.axvspan(datetime.strptime('1996-08-10 15:47:36.00', "%Y-%m-%d %H:%M:%S.%f"), datetime.strptime('1996-08-10 15:47:52.00', "%Y-%m-%d %H:%M:%S.%f"), alpha=0.3, color=interval_color, zorder = zorder_interval_markers)

		ax.legend(fontsize = legend_size, handlelength = 0.2, loc = 'center', bbox_to_anchor=(0.315, 0.25), ncol = 2, frameon = False)
		ax.ticklabel_format(axis="y", style="sci", scilimits=(0,0), useMathText=True)
		...
		ax.fill_between(time.iloc[window_size - 1 + loop_range], markov_noise[3,:], markov_noise[4,:], color = 'orange', alpha = 0.4, zorder=4)
		...
		ax.legend(fontsize = legend_size, ncol=2, handlelength = 0.2, loc = 'center', bbox_to_anchor=(0.2875,0.25), frameon = False)
		ax.ticklabel_format(axis="y", style="sci", scilimits=(0,0), useMathText=True)
		ax.get_yaxis().get_offset_text().set_visible(False)
		ax.text(x_pos_powertext,y_pos_powertext, r'$\times 10^{-2}$', transform=ax.transAxes, fontsize = power_text_factor*text_size)
		...
		ax.set_ylabel('Noise level', fontsize=label_size)
		fig.add_subplot(ax)
...
plt.savefig('Figure_S13_abc.pdf')
plt.show()
