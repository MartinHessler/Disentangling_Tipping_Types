...
def main():
	...
	n_slope_grid_points = 1500
	n_noise_grid_points = 1000
	slope_grid = np.linspace(-50,5,n_slope_grid_points)
	noise_grid = np.linspace(0,1.2, n_noise_grid_points)

	data_model = ds.LangevinEstimation(data, time)
	alpha = tt.time()
	data_model.fast_resilience_scan(window_size, window_shift, slope_grid, noise_grid, 
									n_slope_samples = n_slope_samples, nsteps = nsteps, num_processes = num_processes, 
