...
for i in range(3):
	if i == 0:
		...
		ax.plot(time[window_size + loop_range], markov_slope[0,:], lw = 3, color = 'b', label = r'Estimate $\hat{\zeta}$')
		ax.axvline(time[critical_transition], ls = ':', lw = 3, c = 'r')
		ax.axvspan(time[critical_transition], time[critical_transition + window_size], alpha=0.7, color='grey')
		ax.fill_between(time[window_size + loop_range], markov_slope[1,:], markov_slope[2,:], color = 'orange', alpha = 0.6)
		...
		ax.axvline(time[critical_transition], ls = ':', lw = 3, c = 'r')
		ax.plot(time[window_size::window_shift], markov_noise[0,:], color =  'b', lw = 3, label = r'Estimate $\hat{\sigma}$')
		ax.fill_between(time[window_size::window_shift],markov_noise[1,:], markov_noise[2,:], color = 'orange', alpha = 0.6)
